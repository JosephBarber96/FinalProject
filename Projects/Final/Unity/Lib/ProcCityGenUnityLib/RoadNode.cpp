...
RoadNode::RoadNode(int _xIndex, int _yIndex, Vec2* vec)
	:
	Node(new Vec2(vec->x, vec->y)),
	xIndex(_xIndex),
	yIndex(_yIndex)
{}
...
std::vector<RoadNode*> RoadNode::GetNeighbours(std::vector<std::vector<RoadNode*>> grid)
{
