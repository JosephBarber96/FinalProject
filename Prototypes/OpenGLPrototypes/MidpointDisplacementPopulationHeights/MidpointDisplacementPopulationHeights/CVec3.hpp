...
public:
	...
	CVec3(T xValue, T yValue, T zValue)
	{
		x = xValue;
		y = yValue;
		z = zValue;
	...
	static T dotProduct(const CVec3 &vec1, const CVec3 &vec2)
	{
	...
	T dotProduct(const CVec3 &vec) const
	{
	...
	static CVec3 crossProduct(const CVec3 &vec1, const CVec3 &vec2)
	{
	...
	// }
	//
	static T getDistance(const CVec3 &v1, const CVec3 &v2)
	{
		T dx = v2.x - v1.x;
		T dy = v2.y - v1.y;
		T dz = v2.z - v1.z;
	...
	CVec3 operator+(const CVec3 &vector) const
	{
		return CVec3<T>(x + vector.x, y + vector.y, z + vector.z);
	}

	// Overloaded add and asssign operator to add Vec3s together
	void operator+=(const CVec3 &vector)
	{
		x += vector.x;
		y += vector.y;
		z += vector.z;
	...
	CVec3 operator*(const T &value) const
	{
		return CVec3<T>(x * value, y * value, z * value);
	}
